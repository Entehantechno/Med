#!/usr/bin/env python3
"""Round55 — gynaecology/obstetrics part29 rows 1-220 except 8 deferred.
...
The fixture server/test/fixtures/round55-preservation.json is auto-created on
...
from round55_items import ITEMS
from round55_lessons import LESSONS

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / 'server/test/fixtures/round55-preservation.json'
FIELDS = ('explanation_fa', 'options_why_fa')
MICRO_FIELDS = ('lead_fa', 'golden_fa', 'points_fa')

PARTS = {
    29: 'tools/master-bank/import-payload.master-preint.part29.json',
}
DEFERRED_29 = [59, 103, 110, 132, 166, 183, 191, 218]
EDITED = {29: set(range(1, 221)) - set(DEFERRED_29)}
DEFERRED = {29: set(DEFERRED_29)}
...
def create_fixture():
    ...
    print('fixture created: server/test/fixtures/round55-preservation.json')
    return fixture
...
def enrich():
    ...
    assert set(ITEMS) == {k for p in PARTS for k in [(p, n) for n in EDITED[p]]}, \
        'items keys must cover part29 rows 1..220 except the 8 deferred rows'
    assert all(t in LESSONS for t in {it['topic'] for it in ITEMS.values()}), 'topic gap'

    e29, d29 = enrich_part(29, fixture)
    print(f'PASS: part29 rows 1..220 enriched ({e29} rows, {d29} deferred); '
