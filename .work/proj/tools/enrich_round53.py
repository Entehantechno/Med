#!/usr/bin/env python3
"""Round53 — paediatrics part27 rows 26-220 except 52 deferred + part28 rows 1-5.
...
The fixture server/test/fixtures/round53-preservation.json is auto-created on
...
from round53_items import ITEMS
from round53_lessons import LESSONS

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / 'server/test/fixtures/round53-preservation.json'
...
PARTS = {
    ...
    28: 'tools/master-bank/import-payload.master-preint.part28.json',
}
DEFERRED_27 = [100, 114, 121, 126, 128, 129, 130, 132, 133, 134, 135, 137,
               138, 139, 140, 141, 142, 143, 145, 146, 148, 150, 151, 153,
               154, 156, 160, 161, 162, 163, 164, 166, 167, 169, 170, 171,
               172, 173, 174, 175, 176, 178, 179, 180, 183, 184, 185, 186,
               195, 198, 203, 204]
EDITED = {27: set(range(26, 221)) - set(DEFERRED_27), 28: set(range(1, 6))}
DEFERRED = {27: set(DEFERRED_27), 28: set()}
...
def create_fixture():
    ...
    fixture = {
        'baseline': 'MED-School-100-round52.zip workspace payload state',
    ...
    print('fixture created: server/test/fixtures/round53-preservation.json')
    return fixture
...
def enrich():
    ...
    assert set(ITEMS) == {k for p in PARTS for k in [(p, n) for n in EDITED[p]]}, \
        'items keys must cover part27 rows 26..220 except deferred + part28 rows 1..5'
    assert all(t in LESSONS for t in {it['topic'] for it in ITEMS.values()}), 'topic gap'

    e27, d27 = enrich_part(27, fixture)
    e28, d28 = enrich_part(28, fixture)
    print(f'PASS: part27 rows 26..220 enriched ({e27} rows, {d27} deferred); '
          f'part28 rows 1..5 enriched ({e28} rows, {d28} deferred); '
